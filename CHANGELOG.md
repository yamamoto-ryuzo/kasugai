...
- **Google Maps 2D 移動対応**: CANVAS のカメラ位置・pitch・bearing から Google Maps 2D の表示中心 `@lat,lng` とズームレベルを、提供された CANVAS↔Google Maps ペアに基づく経験式（`s = 2^(0.4*zoom + 0.073*pitch + 7.51)`、`z = 1.057*zoom + 0.0197*pitch - 0.7966`）で算出するようにしました。
