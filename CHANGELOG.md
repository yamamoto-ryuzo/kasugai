...
### 概要
画面1の `🚨 リセット` ボタンと `reinstall_kasugai` コマンドを削除し、updater 経由での更新を一本化しました。

### 変更 (Changed)
- プロジェクトバージョンを `2.7.1` から `2.7.2` に更新。

### 削除 (Removed)
- 画面1の `🚨 リセット` ボタンと `emergencyReinstall()` を削除。
- Rust コマンド `reinstall_kasugai` を削除。updater のキャッシュバスティングで安定しているため、更新フローは updater に一本化。
