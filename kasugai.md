...
#### 配信URL
- **ZIPファイル**: `https://yamamoto-ryuzo.github.io/kasugai/download/kasugai.exe.zip`
- **EXEファイル**: `https://yamamoto-ryuzo.github.io/kasugai/download/kasugai.exe`
...
1. **環境変数の設定**:
   ```powershell
   $env:TAURI_PRIVATE_KEY = "秘密キーの内容"
   $env:TAURI_KEY_PASSWORD = "パスワード"
   ```
...
3. **アーティファクトの準備**:
   - 生成されたEXE: `kasugai/src-tauri/target/release/kasugai.exe`
   - 署名ファイル: `kasugai/src-tauri/target/release/bundle/msi/*.sig`
   - これらを `download/` ディレクトリにコピー

4. **ZIPファイルの作成**:
   ```powershell
   Compress-Archive -Path download/kasugai.exe -DestinationPath download/kasugai.exe.zip
   ```

5. **更新JSONの生成**:
   - `download/latest.json` を手動で更新
   - バージョン、リリースノート、署名、URLを記入

#### GitHub Pagesへのデプロイ
1. `download/` ディレクトリの内容をGitHub Pages用ブランチにコミット
...
#### 3. 秘密キーの管理
- 秘密キーは安全な場所に保管してください
- ビルド時に環境変数 `TAURI_PRIVATE_KEY` と `TAURI_KEY_PASSWORD` で設定します
