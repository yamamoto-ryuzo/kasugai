...
#### 実装アプローチ：
Google MapやRe:Earth、Boxなどの外部サイトのドメインは自身が管理していないため、本来であればクリックイベントをコントロールすることは不可能です。
しかし、本システムではTauriの `initialization_script`（初期化インジェクション機能）を利用し、**画面2（WebView2インスタンス）がページをロードする前に、ページ内のイベントを中書き・乗っ取る特殊なJavaScript**をネイティブ側から埋め込んでいます。

1. **`window.open` のモック化（オーバーライド）:**
   外部サイトのスクリプトが新しいウインドウ（別タブ）を立ち上げようとする `window.open` 命令を乗っ取り、指定URLがTauriスキーム等でなければ、TauriのIPC（`open_in_pane3` コマンド）を実行してイベントをキャンセル。画面3でロードさせます。
2. **クリックイベントのキャプチャリング監視 (`addEventListener` の `useCapture` オン):**
   バブリングを止めるような高度なWebアプリ内でも確実にリンクのクリックを横取りするため、DOMの「キャプチャリングフェーズ」で全てのクリックを監視。
   クリックされた要素に `href`（URL）が存在し、かつそれがローカル環境や自身の初期画面への遷移ではない「外部ドメインへの接続」である場合、リンクのデフォルト動作（画面2での遷移）を `preventDefault()` でブロック。同時にそのURLをTauri API経由でRustに送り、画面3にロードさせます。

これにより、**画面2はメインのWebアプリ（例: Google MapやGISビジュアライザー）の表示状態を100%維持したまま、そこで発見した外部の個別サイトや関連リンクだけが自動的に右側の画面3にストリーミング（展開）されていく**という、マルチディスプレイ環境のような抜群に実用性の高い連携システムを構築しています。
---
