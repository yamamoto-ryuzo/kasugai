...
## 5. Webview制御（Rustロジック詳細）

`src-tauri/src/main.rs` では以下の仕組みを実装しています。

1. **親ウィンドウ (`main`) の作成:**
   Webviewを直接持たない、空の親ウィンドウをサイズ `1200x800` で作成します。
2. **3つの子Webviewの追加:**
   `WebviewBuilder` を用いて、左・中・右それぞれ別のHTML（`index1.html`, `index2.html`, `index3.html`）を指すインスタンスを作成。`add_child` 関数を使って親ウィンドウにマウントします。
3. **ウィンドウのリサイズイベント追従:**
   ユーザーが親ウィンドウをリサイズした際、`WindowEvent::Resized` イベントをキャッチ。
   「現在のウィンドウ幅 $\div$ 3」を各ペインの横幅（`pw`）とし、各Webviewの位置とサイズを `Rect` 構造体を用いて瞬時に再割り当てします（`set_bounds` メソッド）。
