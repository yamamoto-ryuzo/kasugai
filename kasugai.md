...
| **Re:Earth** | Web GIS プラットフォーム、3Dビジュアル統合UI | **【マウント済み（注）】** フロントエンド（Tauri UI / 画面2）にマウントは行っていますが、Re:Earth側のiframeサンドボックスや公開APIの制約により、外部からのカメラ座標やズームの直接的な双方向同期・制御は現状で実現できません（詳細は該当章を参照）。 |
...
本システムは、TauriのマルチWebview機能を最大限に活用し、**境界のドラッグによる滑らかな4画面リサイズ機能**を独自実装しています。

### 5.1. リサイズ自動追従のアーキテクチャと最大化機能

本システムでは、ドラッグによる手動リサイズに加え、**「画面（ペイン）のダブルクリックによる瞬時の最大化・復元機能」**と**「スプリッターのダブルクリックによる表示・非表示トグル機能」**を備えています。

* **各画面（面）のダブルクリック最大化・復元制御:**
  表示されているWebviewの任意の場所をダブルクリックすることで、以下の最大化・復元動作が発火します。
  * **画面1 (`pane1`) は特殊な扱い**:
    画面1はシステム全体のコントロールパネル・リモコンの役割に特化しているため、他の画面のような「全画面最大化」は行いません。ダブルクリックは「開く(固定幅80px)」⇔「閉じる(スプリット化)」の表示・非表示トグルとして動作します。開いている時はウィンドウの拡大・縮小にかかわらず常に「物理幅 80px の最小幅」で完璧に固定されます。
  * **画面2・3・4 ➔ 3つのうち1つを最大化 / 復元**:
    画面2・3・4の最大化は「その3画面のうちどれか1つを大きくする」動作です。**画面1のその時の表示状態（開閉）は保ったまま**、対象のペインだけが残りの領域いっぱいに広がります。
    * **画面2 (`pane2`)**: 画面3・4を閉じて中央を画面2で埋めます。
    * **画面3 (`pane3`)**: スプリッター2を画面1の右端（画面1が閉じていれば左端）まで寄せ、右側全体を画面3にします。
    * **画面4 (`pane4`)**: 画面3を閉じ、中央領域を画面4で埋めます（画面2のタブバーと水平スプリッターは境界として常に残ります）。
    再度ダブルクリックすると、保存されていた比率（`ratio2` / `pane4_ratio`）へ復元されます。復元時も画面1の状態には触れません。
* **スプリッターのダブルクリック ➔ 隣接画面の表示・非表示トグル:**
  外部サイト（Google Map等）を開いている際、サイト内のコンテンツがダブルクリックイベントを吸収してしまう場合があります。これを解決するため、**画面間の境界線（スプリッター）のダブルクリック**では、隣接する画面の表示・非表示を確実にトグルできます。
  * スプリッター1（左の縦境界）をダブルクリック ➔ 画面1の「閉じる」/「最小幅 80px への復元オープン」
  * スプリッター2（右の縦境界）をダブルクリック ➔ 画面3の表示/非表示（非表示前の幅を保存し、再表示時にその幅へ復元。Rustコマンド: `toggle_pane3`）
  * 水平スプリッター（画面2/4間の横境界）をダブルクリック ➔ 画面4の表示/非表示（非表示前の高さを保存し、再表示時にその高さへ復元。Rustコマンド: `toggle_pane4`）
* **境界（スプリッター）の常時表示:**
  どのペインを最大化・非表示にしても、各境界（スプリッター1・2、水平スプリッター）は常に画面内に残るようにレイアウト計算がクランプされています（スプリッター2は左右端で 8px 全幅を確保、画面4最大化時も画面2のタブバー50px＋水平スプリッター8pxを確保）。これにより、どんな状態からでも境界のドラッグ／ダブルクリックでレイアウトを自由に戻せます。
* **ダブルクリック時の誤ドラッグ防止（4pxしきい値）:**
  ダブルクリックは内部的に `mousedown`→微小な `mousemove`→`mouseup` を伴うため、そのままではクリック位置で比率が上書きされてしまいます。これを防ぐため、全スプリッターにおいて**マウスが 4px 以上移動するまでドラッグ扱いにしない**しきい値を設けており、クリック・ダブルクリックでは比率が一切変化しません。
...
* **状態の保存と復元:**
  最大化される直前の画面比率（スプリッターの位置）はシステム内部に保存され、最大化状態でもう一度ダブルクリックすると、元のマルチ画面構成（元の比率）へ瞬時に復元されます。スプリッターダブルクリックによる画面3・4の非表示も同様に、非表示直前の幅・高さが個別に保存され（`saved_pane3_ratio2` / `saved_pane4_ratio`）、再表示時に復元されます。スプリッターを手動でドラッグした場合は手動操作が優先されます。
* **外部サイト上のイベント検知とUI工夫:**
...
* **画面3 (`pane3`)**: Gemini AIアシスタント ＆ 動的タブブラウザ（常に右側の役割・位置）
  ...
  * **タブ操作（画面3）**:
    * **右ダブルクリック**: デフォルトブラウザで開く（外部ブラウザでURLを開く）
    * ※ Gemini以外のタブの場合、ブラウザで開いた後にタブを閉じる
...
### 概要
Kasugaiシステムの配信と自動更新機能をGitHub Pagesを活用して実装しました。これにより、ユーザーは常に最新バージョンを簡単に入手でき、アプリケーション内で自動更新チェックが行われるようになります。

### 配信システム

#### 配信URL
- **インストーラー（NSIS）**: `https://yamamoto-ryuzo.github.io/kasugai/download/kasugai.exe`
- **更新情報JSON**: `https://yamamoto-ryuzo.github.io/kasugai/download/latest.json`

※ 旧 `kasugai.exe.zip` は現在未使用です。

#### 技術構成
- **ビルドシステム**: Tauri v2 + Rust
- **配信プラットフォーム**: GitHub Pages
- **署名**: Tauri署名システム（Minisign）
- **デプロイ**: 手動デプロイ

### 自動更新機能

#### 実装内容
1. **Tauri Updaterプラグイン**: `tauri-plugin-updater` を導入
2. **更新チェック**: 起動時（5秒後）に自動で更新を確認
3. **手動更新**: 設定画面から手動更新チェックが可能
4. **更新通知**: 新しいバージョンが利用可能な場合、ユーザーに通知
...
- **手動更新ボタン**: 即座に更新を確認するボタン
- **更新ステータス**: 更新チェックの結果を表示
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://yamamoto-ryuzo.github.io/kasugai/download/kasugai.exe"
    }
  }
}
```
...
1. **環境変数の設定**:
   ```powershell
   $env:TAURI_SIGNING_PRIVATE_KEY = "秘密キーの内容"
...
3. **アーティファクトの自動配置**:
   `run.py` が自動的に以下を `download/` にコピーし、`latest.json` を生成します。
   - インストーラー: `kasugai/src-tauri/target/release/bundle/nsis/kasugai_*_x64-setup.exe` → `download/kasugai.exe`
   - 署名ファイル: `kasugai/src-tauri/target/release/bundle/nsis/kasugai_*_x64-setup.exe.sig` → `latest.json` の `signature`

#### GitHub Pagesへのデプロイ
1. `download/` ディレクトリの内容をコミット・push
2. GitHub Pages設定で配信を有効化
3. URLが公開されるのを確認
...
- 公開キー: `~/.tauri/mykey.pub`
- 秘密キー: `~/.tauri/mykey`

#### 2. 公開キーの設定
1. `~/.tauri/mykey.pub` の内容をコピー
2. `kasugai/src-tauri/tauri.conf.json` の `pubkey` に貼り付け

#### 3. 秘密キーの管理
- 秘密キーは安全な場所に保管してください
- ビルド時に環境変数 `TAURI_SIGNING_PRIVATE_KEY` と `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` で設定します
- 秘密キーを絶対にリポジトリにコミットしないでください

#### 4. GitHub Pagesの設定
1. GitHubリポジトリのSettings > Pagesを開く
2. Sourceを「Deploy from a branch」に設定
3. Branchを `gh-pages` または適切なブランチに設定
4. `download/` ディレクトリをGitHub Pagesにデプロイする設定を行う

### ユーザー体験

#### 自動更新フロー
1. アプリケーション起動
2. 5秒後に自動で更新チェック（設定で有効な場合）
3. 新しいバージョンが見つかると通知を表示
4. ユーザーが同意するとダウンロード開始
5. ダウンロード完了後に自動インストール
6. アプリケーションが再起動して更新完了

#### 手動更新フロー
1. 設定画面の「アップデート」タブを開く
2. 「今すぐ更新を確認」ボタンをクリック
3. 更新チェックが実行され、結果が表示される
4. 更新がある場合、更新を実行するか選択可能

### セキュリティ考慮事項

- **署名検証**: すべての更新は署名で検証され、信頼できるソースからの更新のみがインストールされます
- **HTTPS通信**: 更新チェックとダウンロードはHTTPS経由で行われます
- **秘密キー管理**: 秘密キーは環境変数で安全に管理し、リポジトリに含めないでください
- **ユーザー同意**: 更新のインストールにはユーザーの明示的な同意が必要です

### トラブルシューティング

#### 更新チェックが失敗する場合
1. インターネット接続を確認
2. GitHub Pagesが正常に動作しているか確認
3. ファイアウォール設定を確認
4. ログでエラーメッセージを確認

#### 署名エラーが発生する場合
1. 公開キーが正しく設定されているか確認
2. 秘密キーの環境変数が正しく設定されているか確認
3. ビルド時に署名が正常に生成されているか確認

### 今後の改善予定

- 複数プラットフォーム対応（macOS、Linux）
- 差分更新の実装（ダウンロードサイズの削減）
- 更新履歴の詳細表示
- 更新スケジュールのカスタマイズ
- バックグラウンド更新の実装
