...
* **Step 5（自己進化：AI主導によるCesium動的プラグインシステム（別途フォルダ拡張））：**
  ...
  - **AIによるプラグイン自動生成とリアルタイム成長:** 画面3の自律AIエージェントが、ユーザーの指示（例：「この3D Tilesに動的ヒートマップを重ね合わせるモジュールを作って」）からCesiumの拡張コードをリアルタイムに自動開発。生成されたJSプラグインファイルを `kasugai_plugins/` へ直接書き出し、Tauriバックエンド（Rust）がこれを検知して実行中のCesiumJSに動的マウント（ホットロード）することで、アプリを一切再起動・再ビルドすることなく「AIによって機能が追加され自己進化するCesium」の完成形を目指します。
