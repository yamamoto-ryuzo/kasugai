...
| **地図同期エンジン** | WebViewのURLリアルタイム解析による緯度経度同期 | 画面間でGoogle MapsやYahoo Map、Google Earth等の現在位置（URLベースの緯度経度・縮尺）を双方向に取得・移動・同期する機能（実装済）。なお、**URL方式の座標認識に対応しているのは Google Maps, Google Earth, Yahoo Map のみ**であり、**CesiumJS と Re:Earth は、それぞれ固有のイベント/メッセージングや専用パラメータ型で座標をやり取りする独自方式**となっています。各地図サービスの仕様差を吸収し、詳細な小数値での高精度同期と、Google Earthの特殊なカメラ距離の標準ズーム値自動変換を実現しています。 |
