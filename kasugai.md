...
* **技術:** `n8n`（ローカルまたはサーバー起動） または `MCP（Model Context Protocol）を活用したGemini API等の直接連携`
* **構成のポイント:**
  * **変化への対応力:** Webシステムの自動化ロジック（例：「Aサイトからデータを取ってBサイトに転記する」）のスケジュールや条件分岐は、Rust内にハードコードするのではなく、**n8nのワークフロー**やAIエージェント（MCPサーバー経由）に委ねます。
  * これにより、Webサービス側の仕様変更があった際も、Rustアプリ自体をビルドし直すことなく、AIのプロンプトやワークフローの調整だけで柔軟に対応できるようになります。
...
* **Step 1（基盤・認証自動化）:** まずは「Tauri + Rust + Playwright + Keyring」で、特定のWebサイトへのID/PASS自動入力と単純なデータ取得・Webログイン連携を確実に動かし、現在のID/PASS連携にかかる手間を削減する。
* **Step 2（空間統合）:** Tauriのフロントに Web GIS（Re:Earth等）を組み込み、地図上の操作からStep 1のPlaywrightが連動して動くようにする（3Dビジュアルとの紐付け）。
* **Step 3（インテリジェンス）:** Rustと n8n や LLM API（MCP）を接続し、トリガーやデータの振り分けをAIに自律判断させる（自律型AI・ワークフローの完全な融合）。
