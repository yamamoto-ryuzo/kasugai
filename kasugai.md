...
本システム最大かつ最も強力なコアアーキテクチャが、**「Rustのネイティブエンジンレベルで WebView2 のナビゲーション（画面遷移）動作そのものを直接傍受・制御するインターセプト機能」**です。
...
```mermaid
graph TD
    A[画面2 /pane2] -- 外部リンクをクリック --> B{Rustネイティブレイヤー<br>on_navigation}
    B -- 1. 画面2の遷移をキャンセル --> C[画面2の地図表示を100%維持]
    B -- 2. バックエンドから強制描画命令 --> D[画面3 /pane3]
    D --> E[指定URLを即座に表示]
...
通常のWebブラウジングにおいて、Google MapsやBox、Re:Earthといった異なる運営元（サードパーティ）のWebサイトに対しては、セキュリティ（Same-Origin Policy、クロスオリジン制約）のために、開発者が外部からJavaScriptフックを埋め込んで他画面と連携させることは構造的に完全に遮断されます。また外部ドメイン上では安全のためTauri独自のAPI（`window.__TAURI__`）の露出も自動的に拒絶されます。

本システムはこのブラウザ制限を、**Web上のJavaScriptを一切使わず、OSネイティブ層（Rust）でWebView2エンジンの最下層を直接ハンドリングする**ことで完全にクリアしました。

1. **`WebviewBuilder::on_navigation` による要求奪取:**
   中央の画面2（`pane2`）を作成する際、Rust側でナビゲーションハンドラを実装。WebView2が外部サーバーと通信を試みる一歩手前で、遷移要求（リクエストURL）をネイティブプロセス側へ完全に横取りします。
2. **高速・セキュアな自動ルーティング判別:**
