...
| **Re:Earth** | Web GIS プラットフォーム、3Dビジュアル統合UI | **【マウント完了】** フロントエンド（Tauri UI / 画面2）にメインビュー/切り替えタブとしてマウント完了（※画面間の位置同期移動などの高度な連携機能は将来ロードマップとし、現在は同期未実施）。 |
...
* **Step 6（Web GIS超融合：Re:Earth × CesiumJS のいいとこ取りハイブリッドシステムの構築）：**
  ...
  - **双方向位置・アングル完全シンクロ（サンドボックスCORS制限の完全突破）:** Re:EarthなどのサードパーティクラウドGISは、iframeやドメイン隔離によるサンドボックス制限（CORS/Same-Origin Policy）があるため、外部からカメラ状態を直接操作することは遮断されます。KASUGAI（Tauri）では、このブラウザの壁をW3C標準のクロスドメインセキュア通信規格である**「`postMessage` ブリッジ（Re:Earthプラグイン連携）」**を介して確実に突破します。Re:Earth側に受信用のシンプルなカスタムプラグインを1つ配置し、親ウィンドウであるTauriから送られてくる位置情報オブジェクトをキャッチさせ、Re:Earth内部のカメラを動かすことで、セッションやセキュリティを1ミリも損なうことなく、Re:EarthとCesiumJSの緯度・経度・高度・カメラアングルを瞬時に連動・シンクロさせます。
