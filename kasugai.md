...
### 概要
Kasugaiシステムの配信と自動更新機能をGitHub PagesとGitHub Actionsを活用して実装しました。これにより、ユーザーは常に最新バージョンを簡単に入手でき、アプリケーション内で自動更新チェックが行われるようになります。
...
#### ビルドとデプロイ
- **トリガー**: `main` ブランチまたは `release` ブランチへのプッシュ
- **ビルド環境**: Windows Runner
- **ビルドステップ**:
  1. リポジトリのチェックアウト
  2. Node.jsとRustのセットアップ
  3. 依存関係のインストール
  4. Tauriアプリケーションのビルド
  5. アーティファクトの準備（EXE、ZIP、署名ファイル）
  6. 更新JSONの生成
  7. GitHub Pagesへのデプロイ

#### 署名プロセス
- ビルド時に自動的に署名ファイル（`.sig`）を生成
- GitHub Secretsで管理された秘密キーを使用
- 更新JSONに署名データを含める
...
- `TAURI_PRIVATE_KEY`: 秘密キーの内容
- `TAURI_KEY_PASSWORD`: 設定したパスワード

#### 4. GitHub Pagesの設定
1. GitHubリポジトリのSettings > Pagesを開く
2. Sourceを「GitHub Actions」に設定
3. ワークフローが正常に実行されると配信が開始されます
...
- **秘密キー管理**: 秘密キーはGitHub Secretsで安全に管理されます
