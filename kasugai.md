...
| :--- | :--- | :--- |
| **CesiumJS** | 3D地球儀プラットフォーム、3Dビジュアル統合UI | **【導入完了】** フロントエンド（Tauri UI / 画面2）にメインビュー/切り替えタブとして統合。Cesium Ion Access TokenのセキュアなKeyring保存と暗号化管理に対応。さらに、ポータル（画面1）と連動した「位置同期移動（`move_cesium`）」を実装。点群データやPLATEAU (3D Tiles) のGPU高速描画に対応。また、重いAssetsやWorkersを外部CDNから読み込む**「CDNハイブリッド構成（`window.CESIUM_BASE_URL`）」**を適用。これにより、EXEファイルを極限まで軽量（数MB〜10MB）に維持しつつ、社外秘のBIM/CADデータやダウンロード済みの巨大なPLATEAUなどの3D Tilesオープンデータを完全ローカルかつセキュアに、ネットワークに情報漏洩させることなく高速描画（プライベート3D描画）するハイブリッドデータ運用が可能です。 |
| **Re:Earth** | Web GIS プラットフォーム、3Dビジュアル統合UI | **【マウント完了】** フロントエンド（Tauri UI / 画面2）にメインビュー/切り替えタブとしてマウント完了（※Tauriインジェクション・メッセージング技術の設計確立により、CORSや二重iframe制限を完全に無効化し、既存の位置同期移動エンジンをRe:Earthに対しても100%有効に適用して連動シンクロさせることが可能となりました）。 |
| **Rust 空間データエンジン** | 空間データハンドリング、重い3DファイルのI/O制御 | ローカルにある点群（LAS/LAZ）、BIMモデルなどの巨大なファイルをRust側で高速解析・監視・ストリーミング処理 |
| **地図同期エンジン** | WebViewのURLリアルタイム解析による緯度経度同期 | 画面間でGoogle MapsやYahoo Map、Google Earth等の現在位置（URLベースの緯度経度・縮尺）を双方向に取得・移動・同期する機能（実装済）。各地図サービスの仕様差を吸収し、詳細な小数値での高精度同期と、Google Earthの特殊なカメラ距離の標準ズーム値自動変換を実現。 |
...
* **Step 5（自己進化：AI主導によるCesium動的プラグインシステム（別途フォルダ拡張））：**
  - **コア（EXE）と拡張（別途フォルダ）の分離:** プラットフォーム本体（`kasugai.exe`）は基本エンジンとして内包・固定（不変）とし、機能拡張用の別途フォルダ `kasugai_plugins/` をアプリの隣に構築。起動時にこの別途フォルダ内のJS拡張モジュールを動的にインポート・結合する**プラグイン型アーキテクチャ**をロードマップに定義します。
  - **AIによるプラグイン自動生成とリアルタイム成長:** 画面3の自律AIエージェントが、ユーザーの指示（例：「この3D Tilesに動的ヒートマップを重ね合わせるモジュールを作って」）からCesium of 拡張コードをリアルタイムに自動開発。生成されたJSプラグインファイルを `kasugai_plugins/` へ直接書き出し、Tauriバックエンド（Rust）がこれを検知して実行中のCesiumJSに動的マウント（ホットロード）することで、アプリを一切再起動・再ビルドすることなく「AIによって機能が追加され自己進化するCesium」の完成形を目指します。
* **Step 6（Web GIS超融合：Re:Earth × CesiumJS のいいとこ取りハイブリッドシステムの構築）：**
  - **Re:Earth（ノーコード公開）とCesiumJS（高度シームレス処理）の双方向連携:** Re:Earthが持つ「ノーコードで誰でも一瞬で美しいWeb 3Dマップを構築し、外部へ一般公開・共有できる強み」と、素のCesiumJSが持つ「ローカルのBIM/CADやLAS点群の読み込み、高度なシミュレーションを100%自由に開発できる強み」をTauriのマルチWebview上で融合します。
  - **双方向位置・アングル完全シンクロ（サンドボックスCORS制限の完全突破）:** Re:EarthなどのサードパーティクラウドGISは、iframeやドメイン隔離による強力なサンドボックス制限（CORS/Same-Origin Policy）があり、プラグイン自体も内部iframeに隔離されるため、通常の `postMessage` ではメッセージが遮断されます。KASUGAI（Tauri）では、この隔離をOS・Webviewエンジンレベルの権限である**「Tauriインジェクション・メッセージング・パイプライン」**によって完全に突破します。TauriがRe:Earthのロード直前に最外層（Windowコンテキスト）へ中継ブリッジJSコードを強制注入（`initialization_script`）。プラグイン用の隔離iframeを完全にスルーして最外層に露出している `window.reearth` オブジェクトのカメラAPIを直接操作・動的実行（`eval`）することで、どのような隔離セキュリティ下でも100%確実に、セッションや安全性を保ったままRe:EarthとCesiumJSの位置・ズームアングルを完全にシンクロさせます。
  - **パブリッシュ・データパイプラインの自動化:** CesiumJS上で機密データを扱いながら、AI（画面3）が「外部公開用に抽象化・軽量化したGeoJSONや3Dポリゴンデータ」を自動抽出し、Tauri（Rust）を介して直接Re:Earthの公開プロジェクト（Web）へ動的にパブリッシュ（データアップロード）する連動パイプラインを構築します。
  - **透過オーバーレイ・マルチレイヤ（究極の合成）:** TauriのマルチWebview特性を活かし、背景に「Re:Earthのノーコード標準ビュー」を描画しつつ、その上に背景を透過させた「ローカルCesiumの高度シミュレーション・点群レイヤー」のWebviewをミリ秒単位で完全に重ね合わせて同時描画。ユーザーからは「Re:Earthのウェブ公開UIの内部で、直接ローカルの大容量点群や機密BIMがグリグリ動いている」ように見える究極のGIS合成ビュー（いいとこ取り）を実現します。
