...
* **各画面のダブルクリック最大化:**
  表示されているWebview（画面1、画面2、画面3など）の任意の場所をダブルクリックすることで、そのペインが1画面に最大化されます。
  * 画面1（`pane1`）をダブルクリック ➔ 左画面が最大化
  * 画面2（`pane2`等）をダブルクリック ➔ 中央画面が最大化
  * 画面3（`pane3`）をダブルクリック ➔ 右画面が最大化
* **状態の保存と復元:**
...
* **外部サイト上のイベント検知:**
  Tauriの `initialization_script` 機能を用いて、各Webviewのロード時にネイティブレベルでJavaScriptを自動注入（インジェクト）しています。これにより、Google MapsやBoxなどの外部ドメイン（クロスオリジン）を表示している最中であっても、ダブルクリックを正確に検知してRustバックエンドへ通知し、シームレスな最大化制御を実現しています。
...
※ 「表示切替」を行った場合でも、画面2（pane2）および画面3（pane3）に設定されているナビゲーションルール（役割）はそのまま固定されて維持されます。
...
### 6.4. 【重要】「画面識別子（1・2・3）」と「表示切替」時における役割・位置の固定仕様

本システムでは、画面1の「🔄 表示切替」ボタンをクリックすることで、画面を再読込することなく高速に中央（画面2）と右（画面3）の表示位置をスワップできます。このとき、画面2.3としてのプログラム上の役割（サンドボックス制限など）と、起動時の立ち上げ位置関係は以下のように固定されて維持されます。

#### ① 実装（プログラム）上の固定定義（専用WebViewプーリング構成）
...
* **画面2レイヤー (中央表示用のWebView群)**:
  ...
  * **`pane2_google` (Google Map専用)**: Google Map専用WebView。
  * ※ これらはすべて同時にメモリ上に存在しており、ユーザーがポータルボタンを押すたびにアクティブなWebViewの表示座標（`bounds`）のみを書き換えて前面に表示し、非アクティブなものは不可視領域（`-10000, -10000`）へ退避させることで、**各サービスの内部状態（ログインセッションや地図の現在位置など）を完全に維持したまま瞬時に画面を切り替える**極めて強力な構成となっています。
* **画面3 (`pane3`)**: Rustバックエンド連携（常に右側の役割・位置）
  * ※ BOXなど、外部ドメインへのリダイレクトやログイン認証が必要なサービスを開くための自由な参照ビューとして使用します。

#### ② 表示切替（スワップ）時の挙動
表示切替ボタンをクリックした際、再読み込みを伴わない高速な切り替えを実現するため、WebView自体の物理的な配置（座標 bounds）を瞬時に入れ替えています。

ただし、**「画面2.3としての役割とポータルリンク起動時の立ち上げ位置」は常に固定**されます：
* **中央画面への固定起動**: 画面が切り替わっている状態（スワップ中）であっても、画面1（ポータル）から「BOX」や「Re:Earth」を起動した際には、現在物理的に中央に配置されている側のWebView（スップ時は `pane3`）へとURLが自動的に流し込まれ、**常に中央の画面で綺麗に立ち上がる**ように制御されています。
* **ナビゲーション傍受の固定**: 
  - `pane2` は物理的な表示位置が右側に移動しても、同一ホスト内のみ遷移を許可するサンドボックスとしての役割を維持します。
  - `pane3` は物理的な表示位置が中央に移動しても、自由なブラウジング領域としての役割を維持します。
...
- **起動時（初期遷移時）の自動ログイン保証**: 専用WebView（`pane2_box`, `pane2_reearth`）の初期URLを `about:blank` に設定し、アプリ起動時のバックグラウンド裏読み（プレロード）を意図的にスキップする設計としています。これにより、ユーザーが最初にポータルボタンをクリックして実際に画面へ切り替わったタイミングでのみ自動ログイン処理（DOM操作や物理タイピング）が確実に発火し、意図しないタイミングでのパスワード入力や誤作動を完全に防止しています。
...
- **ペインラベルの明示的なターゲット指定**: 
  画面のスワップ（中央と右の入れ替え）に関わらず、目的のWebView（`pane2` / `pane3`）をラベルで直接狙い撃ちすることで、常にユーザーの直感に一致した自動化動作を保証します。
