...
[概要動画はこちら](https://youtu.be/wEtE6rjQa9I)
...
### 2.2. マルチ・ウィンドウによる統合UIレイヤー
Tauriのマルチウィンドウ管理を活用し、タスクごとに最適化されたワークスペースを並列提供します。
- **空間可視化（デジタルツイン・GIS）**: `Re:Earth`や`Google Maps`を統合し、地理空間コンテキスト上で全業務データを可視化します。
- **CDE（共通データ環境）管理**: `Box`等のクラウドストレージやLAN上のSamba共有ファイルをネイティブアプリの速度で直接操作。デジタルツイン上のオブジェクトとのリアルタイム連動を実現します。
- **対話型コンソール**: 自然言語によるプラットフォーム横断タスク（例：「現在進行中のプロジェクト最新報告書を3Dマップ上に展開」等）をオーケストレーションする専用のAIインターフェースです。
...
現在、手動による外部WEBシステムのID/PASS連携に手間を要している課題を解消し、Kasugai本来の趣旨である「繋ぐ（鎹）」役割を極限まで進化させるため、以下の将来開発方針を掲げています。

ベースシステムが**Rust**であり、かつ**デスクトップアプリ**という強力な足回りを持つ強みを最大限に活かし、**「自律型AI・ワークフロー連携」**と**「Web GISによる3D視覚的統合（デジタルツイン）」**の両方を内包したシステムを目指します。
...
#### ③ AI・自律化レイヤー（インテリジェンス）
* **技術:** `n8n`（ローカル・サーバー） または `MCP (Model Context Protocol) 経由のLLM連携`
* **概要:** Web自動化ロジックのスケジュールや条件分岐をAI（MCPサーバー/n8n）に委ねることで、外部Webシステムの仕様変更時も、デスクトップアプリを再コンパイルすることなくプロンプトやワークフローの調整だけで対応できる柔軟性を確保します。
...
* **Step 3（インテリジェンス）:** Rustと n8n / LLM API（MCP）を接続し、トリガーやデータ振分けをAIが自律判断・進化する仕組みを構築する。
